...
public class UserServiceImpl implements UserService {
    ...
    public void updateAvatar(long userId, String avatarUrl) {
        // 同步更新两个表，确保资料页与会话信息能读取到头像
        userMapper.updateUserAvatar(userId, avatarUrl);
        com.xiaoRed.entity.auth.Account account = userMapper.findAccountById(userId);
        String username = account != null ? account.getUsername() : String.valueOf(userId);
        userMapper.upsertInfoAvatar(userId, username, avatarUrl);
    }

    @Override
    public boolean isValidUser(long userId, String username) {
        // 使用 db_account 表校验，避免与 db_account_user 不一致
        com.xiaoRed.entity.auth.Account account = userMapper.findAccountById(userId);
