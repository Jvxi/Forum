...
public interface UserMapper {
    ...
    @Select("select a.id, a.username, a.email, ifnull(i.avatar, '') as avatar from db_account a left join db_account_info i on a.id = i.uid where a.username = #{text} or a.email = #{text}")
    ...
    @Insert(" insert into db_account_info(uid, username, sex, phone, qq, wx, blog, `desc`) " +
            "values(#{uid}, #{username}, #{sex}, #{phone}, #{qq}, #{wx}, #{blog}, #{desc}) " +
            "on duplicate key update uid=#{uid}, username=#{username}, sex=#{sex}, phone=#{phone}, " +
    ...
    @Update("update db_account_user set avatar = #{avatarUrl} where id = #{userId}")
    void updateUserAvatar(long userId, String avatarUrl);
    
    @Insert("insert into db_account_info(uid, username, avatar) values (#{userId}, #{username}, #{avatarUrl}) on duplicate key update avatar = #{avatarUrl}")
    void upsertInfoAvatar(long userId, String username, String avatarUrl);
    
    @Select("select a.id, a.username, a.email, ifnull(i.avatar, '') as avatar from db_account a left join db_account_info i on a.id = i.uid where a.id = #{id}")
    AccountUser findAccountUserById(long id);
}
