    ...
    <el-container style="height: 100%">
      ...
      <el-aside class="content-aside" :style="{width : hideMenu ? '0': '270px'}">
        ...
        >
          ...
          <el-menu-item index="/index/likes">
...
const logout = ()=>{
  ...
  get('/api/auth/logout',(message) =>{//退出登录成功
    ...
    router.push('/').then(() => location.reload()); // 强制刷新页面
  })
}
</script>
...
.header-right {
  display: flex;
  align-items: center;
  margin-left: auto;  /* 关键属性：将右侧内容推到最右边 */
  gap: 15px;          /* 元素间距 */
}
...
.content-aside{
  ...
  transition: width 0.3s ease, opacity 0.3s ease;/*属性变化（侧边栏折叠）不立即生效，而是有一个0.3s的过渡时间*/
}
